...
export const PipelineFlowVisualizer: React.FC = () => {
  const pipelineNodes = [
    ...
    },
    {
      ...
      desc: "Combines 7 feeds into DistrictSnapshots enriched with Census DB",
      icon: Layers,
      badge: "PURE PYTHON",
      color: "border-zinc-700 text-zinc-300 bg-zinc-800",
    },
  ...
  return (
    <div className="bg-panel border border-hud rounded-sm p-4 space-y-4 text-xs font-sans">
      <div className="flex items-center justify-between pb-3 border-b border-hud">
        ...
        <span className="font-mono text-xs text-zinc-400 bg-black/40 px-2 py-0.5 rounded-sm border border-hud">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 font-mono text-xs">
        {pipelineNodes.map((node, idx) => {
          ...
          return (
            <div
              key={idx}
              className="bg-panel-container border border-hud p-3.5 rounded-sm space-y-2 relative group hover:border-zinc-500 transition-all"
            >
              <div className="flex items-center justify-between">
                ...
                <span className={`px-2 py-0.5 rounded-sm border text-[10px] font-semibold ${node.color}`}>
                  {node.badge}
                </span>
              </div>
      ...
      {/* Rules & Mandates Box */}
      <div className="bg-black/40 border border-hud p-3.5 rounded-sm font-mono text-xs space-y-1.5 text-zinc-300">
        <div className="font-semibold text-white text-xs uppercase flex items-center gap-1.5 border-b border-hud pb-1.5">
