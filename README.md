...
> Built as a hackathon prototype. See [Limitations](#limitations--known-issues) before treating this as production-ready.
...
- [Key Design Principle: Deterministic Risk, AI Explanation](#key-design-principle-deterministic-risk-ai-explanation)
...
Then open `.env.local` and set your Gemini API key (see [Environment Variables](#environment-variables) below).
...
1. **Incident Reports** — per-district, with severity level, evidence footprint, and Gemini/fallback narrative.
2. **District Priority Report** — strictly risk-score-ranked district list for resource triage.
3. **Resource Request Summary** — transparent, formula-based resource math (e.g. `ORS = Hospital Score × Population Weight × 8`, `Test Kits = Disease Score × 20`), inter-district medicine redistribution suggestions, and emergency camp mobilization triggers.
...
For a full phase-by-phase history of how this system was built, see [CHANGELOG.md](./CHANGELOG.md).
