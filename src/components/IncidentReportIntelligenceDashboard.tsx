...
> = ({ summary, cycleState, onSelectDistrict }) => {
  ...
  return (
    <div className="bg-panel border border-hud rounded-sm p-4 space-y-5 font-mono text-xs">
      ...
      {/* 2.5 CLINICAL INTELLIGENCE */}
      {clinical && (
        <div className="bg-panel-container border border-red-500/30 rounded-sm p-4 space-y-3">
          <div className="flex items-center gap-2 border-b border-red-500/30 pb-2">
            <Stethoscope className="w-4 h-4 text-red-400" />
            <h3 className="font-display font-bold text-xs text-red-300 uppercase tracking-wider">
              CLINICAL INTELLIGENCE: {clinical.likely_disease.toUpperCase()}
            </h3>
          </div>
          <div className="grid grid-cols-2 gap-4 text-xs">
            <div>
              <span className="text-gray-400 font-bold">Transmission:</span>
              <p className="text-white">{clinical.transmission}</p>
            </div>
            <div>
              <span className="text-gray-400 font-bold">Incubation:</span>
              <p className="text-white">{clinical.incubation}</p>
            </div>
            <div className="col-span-2">
              <span className="text-gray-400 font-bold">Likely Symptoms:</span>
              <p className="text-white">{clinical.symptoms.join(", ")}</p>
            </div>
            <div className="col-span-2 bg-red-950/20 p-2 rounded border border-red-900/50">
              <span className="text-red-300 font-bold">Public Advisory:</span>
              <p className="text-red-100">{clinical.public_advisory}</p>
            </div>
          </div>
        </div>
      )}
